...
export const TABLES: Record<string, TableDef> = {
  ...
  // A separate service with its own SP registry, DataCap+SLI proof, and one
  ...
  porep_rail_id_updated: {
    description: "Links a PoRep deal to its FilecoinPay rail. Primary bridge: rail_id joins fp_rail_created / fp_rail_settled; deal_id joins the porep_deal_* tables. Lets fp_* revenue/settlement analytics attribute to PoRep deals, clients, and SPs",
    columns: {
      dealId: { type: "bigint" },
      railId: { type: "bigint" },
    },
  ...
  porep_proxy_created: {
    description: "PoRep ValidatorFactory deployed a per-deal Validator (the FilecoinPay operator for that deal). proxy = fp_rail_created.operator; deal_id links to porep_deal_*. Discovers every per-deal operator address",
    columns: {
  ...
  porep_sp_payee_updated: {
    description: "PoRep SP payee address changed. newPayee = fp_rail_created.payee; providerId = SP Filecoin actor id. Resolves rail payees to PoRep SPs. Fires only on change: an SP whose initial custom payee never changes needs a getPayee(providerId) RPC read; otherwise porep_sp_registered.organization is the payee",
    columns: {
      providerId: { type: "bigint", note: "SP Filecoin actor id" },
      oldPayee: { type: "hex" },
      newPayee: { type: "hex" },
  ...
  porep_sp_price_updated: {
    description: "PoRep SP price changed. newPrice = price per 32GiB sector per month, raw integer. Deal rate = price * sectorCount / 86400 epochs. Payment token is chosen per deal at createRail (mainnet: USDFC or axlUSDC), so scale by the deal's rail token decimals (USDFC 1e18, axlUSDC 1e6), not a fixed 1e18",
    columns: {
      providerId: { type: "bigint", note: "SP Filecoin actor id" },
  ...
  porep_sli_attestation_update: {
    description: "PoRep SLI oracle attestation for an SP. The four SLI columns are the measured values used to evaluate deal requirements during settlement; a below-threshold attestation pays zero for the settlement interval. lastUpdate is the Filecoin epoch of the attestation",
    columns: {
      providerId: { type: "bigint", note: "SP Filecoin actor id" },
